...
function calculateBurnoutRiskIndex(
  dailyLoads,
  metricsTable,
  date,
  bodyParts,
...
function fatigue(intensity, duration, daysLater) {
  ...
  return (
    intensity * duration * Math.pow(decayFactor, daysLater / daysToRecovery)
  );
}
...
function incrementFatigues(fatigues, date, intensities, durationInHours) {
  for (let daysLater = 1; daysLater <= 14; daysLater++) {
    ...
    fatigues[fatigueDate].fingers += fatigue(
      intensities.fingers,
      durationInHours,
      daysLater
    );
    fatigues[fatigueDate].upperBody += fatigue(
      intensities.upperBody,
      durationInHours,
      daysLater
    );
    fatigues[fatigueDate].lowerBody += fatigue(
